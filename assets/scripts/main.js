...
async function getRecipes() {
  ...
  // A3. Return a new Promise. If you are unfamiliar with promises, MDN has a great article on them.
  // A promise takes one parameter - A function (we call these callback functions).
  // That function will take two parameters - resolve, and reject. These are functions
  // you can call to either resolve the Promise or Reject it.
  return new Promise((resolve, reject) => {
    // A4-A11 will all be *inside* the callback function we passed to the Promise we're returning
    ...
    RECIPE_URLS.forEach(async (url) => {
      ...
      // A6-A9 will be in the try portion, A10-A11 will be in the catch portion.
      try {
        // A6. For each URL in that array, fetch the URL - MDN also has a great article on fetch().
        // NOTE: Fetches are ASYNCHRONOUS, meaning that you must either use "await fetch(...)" or "fetch.then(...)".
        // This function is using the async keyword so we recommend "await"
        const response = await fetch(url);
        // A7. For each fetch response, retrieve the JSON from it using .json().
        // NOTE: .json() is ALSO asynchronous, so you will need to use "await" again
